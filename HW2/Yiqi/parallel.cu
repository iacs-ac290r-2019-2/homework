...
__global__ void jacobi_solver(double* u, double* u_new, double pref, int nx, int ny) {
  ...
  if (ti < nx && tj < ny) {
    s_u[si + sj*blockDim.x] = u[ti + tj*nx];
  }
  ...
  if ((ti >= 1 && ti < (nx-1)) && 
    ...
    // printf("(si, sj) = (%d, %d): %.5lf\n", si, sj, s_u[si + sj*blockDim.x]);
    // printf("(ui, uj) = (%d, %d): %.5lf -> %.5lf\n", ti, tj, u[ti + tj*nx], u_new[ti + tj*nx]);
  }
...
__global__ void check_error(double* u, double* error, double D, double time, double dx, double dy, int nx, int ny) {
  ...
  if (ti < nx && tj < ny &&
    ...
    error[ti + tj*nx] = u[ti + tj*nx] / (sin(ti*dx) * sin(tj*dy) * exp(-2*D*time)) - 1;
  }
...
int main(int argc, char *argv[])
{
  ...
  // printf("t* = %.5lf: ", 0.);
  // printf("Error = %e, ", error/(nx*ny));
  // printf("Center = %e\n", 
  //         0.25*(u[(nx+1)/2 + ((ny+1)/2)*nx] + 
  //               u[(nx/2+1) + ((ny+1)/2)*nx] + 
  //               u[(nx+1)/2 + (ny/2+1)*nx] + 
  //               u[(nx/2+1) + (ny/2+1)*nx]));
  ...
  for (double time = 0.; time <= sim_time; time += dt) {
    ...
    jacobi_solver<<<num_blocks, dim_blocks>>>(_u, _u_new, pref, nx, ny);
    ...
    check_error<<<num_blocks, dim_blocks>>>(_u_new, _error, D, time, dx, dy, nx, ny);
    ...
    printf("Error at t* = %.5lf is %e\n", (time+dt)*D/(Lx*Lx), error/(nx*ny));

    // cudaMemcpy(u, _u_new, nx*ny * sizeof(double), cudaMemcpyDeviceToHost);
    // printf("t* = %.5lf: ", (time+dt)*D/(Lx*Lx));
    // printf("Error = %e, ", error/(nx*ny));
    // printf("Center = %e\n", 
    //         0.25*(u[(nx+1)/2 + ((ny+1)/2)*nx] + 
    //               u[(nx/2+1) + ((ny+1)/2)*nx] + 
